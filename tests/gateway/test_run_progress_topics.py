...
class ProgressAcrossContentAgent:
    def __init__(self, **kwargs):
        self.tool_progress_callback = kwargs.get("tool_progress_callback")
        self.interim_assistant_callback = kwargs.get("interim_assistant_callback")
        self.tools = []
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        self.tool_progress_callback("tool.started", "terminal", "first command", {})
        time.sleep(0.45)
        self.interim_assistant_callback("interim content")
        time.sleep(0.45)
        self.tool_progress_callback("tool.started", "browser_navigate", "https://example.com", {})
        # The progress sender throttles edits; emit one more tool after the
        # throttle window so the accumulated post-content progress is flushed.
        time.sleep(1.7)
        self.tool_progress_callback("tool.started", "execute_code", None, {"code": "print('done')"})
        time.sleep(0.45)
        return {
            "final_response": "done",
            "messages": [],
...
class DuplicateProgressAcrossContentAgent:
    def __init__(self, **kwargs):
        self.tool_progress_callback = kwargs.get("tool_progress_callback")
        self.interim_assistant_callback = kwargs.get("interim_assistant_callback")
        self.tools = []
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        self.tool_progress_callback("tool.started", "terminal", "same command", {})
        time.sleep(0.45)
        self.interim_assistant_callback("interim content")
        time.sleep(0.45)
        self.tool_progress_callback("tool.started", "terminal", "same command", {})
        # Emit a distinct post-throttle tool so the sender flushes any
        # content-separated duplicate state through the normal edit path.
        time.sleep(1.7)
        self.tool_progress_callback("tool.started", "browser_navigate", "https://example.com", {})
        time.sleep(0.45)
        return {
...
@pytest.mark.asyncio
async def test_run_agent_progress_edits_single_bubble_across_content_segments(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_TOOL_PROGRESS_MODE", "all")
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        ProgressAcrossContentAgent,
        session_id="sess-progress-across-content",
        config_data={
            "display": {
                "tool_progress": "all",
                "interim_assistant_messages": True,
    ...
    progress_sends = [
        ...
        if (
            "terminal:" in call["content"]
            or "browser_navigate" in call["content"]
            or "execute_code" in call["content"]
        )
    ]
    assert [call["content"] for call in progress_sends] == [
        '💻 terminal: "first command"',
    ]
    assert any(call["content"] == "interim content" for call in adapter.sent), adapter.sent
    assert any(
        call["message_id"] == "progress-1"
        and '💻 terminal: "first command"' in call["content"]
        and "browser_navigate" in call["content"]
        and "https://example.com" in call["content"]
        and "execute_code" in call["content"]
        for call in adapter.edits
    ), f"sent={adapter.sent!r} edits={adapter.edits!r}"


@pytest.mark.asyncio
async def test_run_agent_progress_does_not_dedup_across_content_segments(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_TOOL_PROGRESS_MODE", "all")
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        DuplicateProgressAcrossContentAgent,
        session_id="sess-progress-dedup-across-content",
        config_data={
            "display": {
                "tool_progress": "all",
                "interim_assistant_messages": True,
    ...
    progress_edits = [
        call["content"]
        for call in adapter.edits
        if '💻 terminal: "same command"' in call["content"]
    ]
    assert progress_edits, f"sent={adapter.sent!r} edits={adapter.edits!r}"
    final_progress = progress_edits[-1]
    assert final_progress.count('💻 terminal: "same command"') == 2
    assert "×2" not in final_progress


@pytest.mark.asyncio
async def test_run_agent_progress_does_not_dedup_across_direct_interim_messages(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_TOOL_PROGRESS_MODE", "all")
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        DuplicateProgressAcrossContentAgent,
        session_id="sess-progress-dedup-direct-interim",
        config_data={
            "display": {
                "tool_progress": "all",
                "interim_assistant_messages": True,
